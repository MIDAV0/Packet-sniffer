...
void sniff(char *interface, int verbose) {
  ...
  // Catch ctrl-c signal and close the pcap session
...
// It closes the pcap session and exits the program
void signal_handler(int signo){
  if (signo == SIGINT){
    // Break the pcap_loop and close the session
    if (pcap_handle != NULL){
        pcap_breakloop(pcap_handle);
        pcap_close(pcap_handle);
      }

    // Kill the threads and print report
    kill_threads();
